...
const ImageWithLoad = ({ src, alt }: { src: string, alt: string }) => {
  ...
  return (
    <div className="mt-4 rounded-xl overflow-hidden border border-gray-800 shadow-sm bg-gray-900 relative min-h-[300px] flex items-center justify-center">
      {!loaded && !error && (
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-gray-100 bg-black">
      
      {/* AUTHENTICATION GATE */}
      <SignedOut>
        <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-black">
          <div className="mb-8 text-center">
            <h1 className="logo-text text-5xl mb-4 text-white">Xodiac AI</h1>
            <p className="text-gray-400 max-w-md mx-auto">
              Sign in to access your personal AI research assistant powered by Gemini and Grok.
            </p>
          </div>
          <div className="bg-[#111] p-1 rounded-2xl border border-gray-800">
             <SignIn appearance={{
                elements: {
                    rootBox: "w-full",
                    card: "bg-[#111] border-none shadow-none text-white",
                    headerTitle: "text-white",
                    headerSubtitle: "text-gray-400",
                    socialButtonsBlockButton: "bg-[#222] border-gray-700 text-white hover:bg-[#333]",
                    socialButtonsBlockButtonText: "text-white",
                    dividerLine: "bg-gray-700",
                    dividerText: "text-gray-400",
                    formFieldLabel: "text-gray-300",
                    formFieldInput: "bg-[#222] border-gray-700 text-white",
                    footerActionText: "text-gray-400",
                    footerActionLink: "text-blue-400 hover:text-blue-300",
                    identifierInput: "bg-black"
                }
             }} />
          </div>
        </div>
      ...
      <SignedIn>
        <div className="flex min-h-screen">
            ...
            <main className={`flex-1 flex flex-col pl-[72px] transition-all duration-500`}>
                
                {!hasStarted && (
                <div className="flex flex-col items-center justify-center min-h-screen px-4 w-full -mt-10 fade-in">
                    <h1 className="logo-text text-5xl mb-8 text-white">Xodiac AI</h1>
                    <div className="w-full flex justify-center">
                        <SearchBar onSearch={handleSearch} isLoading={isLoading} isCentered={true} />
                    </div>
                    
                    <div className="fixed bottom-8 right-8 flex space-x-4 text-gray-500">
                ...
                <div className="w-full max-w-[750px] mx-auto px-4 md:px-0 pt-8 pb-40">
                    ...
                    <header className="fixed top-0 left-[72px] right-0 bg-black/95 backdrop-blur-sm z-30 border-b border-gray-800">
                    <div className="max-w-[750px] mx-auto flex items-center justify-start h-16 px-4 md:px-0">
                    ...
                    {messages.map((msg, idx) => (
                        ...
                        {msg.role === 'user' ? (
                            <div className="text-[32px] font-medium text-gray-100 tracking-tight mb-8 mt-6 leading-tight">
                            {msg.text}
                            </div>
                        ) : (
                            ...
                            {msg.sources && msg.sources.length > 0 && (
                                ...
                                <div className="flex items-center space-x-2 mb-3">
                                    ...
                                    <span className="text-base font-medium text-gray-200">Sources</span>
                                </div>
                                ...
                                {(!msg.isThinking || getDisplayText(msg.text).length > 0) && (
                                    <div className="flex items-center space-x-2 mb-3">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            {msg.isDeepThink && (
                                                <span className="text-xs font-medium text-blue-400 bg-blue-900/30 px-2 py-0.5 rounded-full border border-blue-800/50">Deep Think</span>
                                            )}
                                        </div>
                                    </div>
                                )}
                            ...
                            {!msg.isThinking && msg.relatedQuestions && msg.relatedQuestions.length > 0 && (
                                <div className="mt-6 pt-2 border-t border-gray-800">
                                    <div className="flex items-center space-x-2 mb-3 mt-4">
                                        ...
                                        <span className="text-lg font-medium text-gray-200">Related</span>
                                    </div>
                                    <div className="space-y-2">
                                        {msg.relatedQuestions.map((question, i) => (
                                            <div key={i} className="group flex items-center justify-between p-3 hover:bg-gray-900 rounded-lg cursor-pointer border-b border-gray-800 last:border-0 transition-colors" onClick={() => handleSearch(question, false)}>
                                                <span className="text-gray-400 font-medium group-hover:text-[#14b8a6]">{question}</span>
                                                <div className="text-gray-600 group-hover:text-[#14b8a6] transition-colors">+</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <div className="fixed bottom-0 left-[72px] right-0 bg-gradient-to-t from-black via-black to-transparent pt-12 pb-6 z-20">
